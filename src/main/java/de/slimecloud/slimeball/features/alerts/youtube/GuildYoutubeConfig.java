...
public class GuildYoutubeConfig extends ConfigCategory {
	...
	@NotNull
	public Optional<Long> getRoleId(@NotNull String youtubeChannelId) {
		return Optional.ofNullable(role.get(youtubeChannelId));
	}

	@NotNull
	public Optional<Role> getRole(@NotNull String youtubeChannelId) {
		return getRoleId(youtubeChannelId).flatMap(id -> Optional.ofNullable(bot.getJda().getRoleById(id)));
	}
}
